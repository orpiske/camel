...
The body can either contain the user prompt as a String along with the *CamelLangChain4jAgentSystemMessage* header for the system prompt, or you can create an object of type *org.apache.camel.component.langchain4j.agent.api.AiAgentBody* containing both *userMessage* and *systemMessage*.
...
The memory works for multiple users/sessions. For each context window, the users need to set the memory ID:
- By setting the Header CamelLangChain4jAgentMemoryId. This supposes that a user is using the body as String.
