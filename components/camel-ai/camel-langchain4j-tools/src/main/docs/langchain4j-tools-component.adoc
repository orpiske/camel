...
Consider, for instance, two consumer routes capable of querying a user database by user ID or by social security number (SSN).
...
Consumer routes must define tags that group https://en.wikipedia.org/wiki/Set_theory[together].
The aforementioned routes would be part have the `users` tag.
The `users` tag has two routes: `queryById` and `queryBySSN`
