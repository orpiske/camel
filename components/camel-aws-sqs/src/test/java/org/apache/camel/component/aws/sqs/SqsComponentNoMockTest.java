...
public class SqsComponentNoMockTest extends CamelTestSupport {
    ...
    @Before
    public void setup() {
        client = AmazonSQSClientBuilder.standard().withCredentials(new AWSCredentialsProvider() {
            @Override
    ...
    protected RouteBuilder createRouteBuilder() throws Exception {
        final String queue = System.getProperty("aws.sqs.queue");
        final String accessKey = System.getProperty("aws.access.key");
        final String secretKey = System.getProperty("aws.secret.key");

        return new RouteBuilder() {
            final String sqsURI = String.format("aws-sqs://%s?accessKey=%s&secretKey=%s&region=US_WEST_2&amazonAWSHost=amazonaws.com&messageRetentionPeriod=%s&maximumMessageSize=%s",
                    queue, accessKey, secretKey, "1209600", "65536");


            @Override
            public void configure() throws Exception {
                System.out.println("Using queue: " + sqsURI);
